...
const questions = [
  ...
  () =>
    "What outcome should change for the user and for the business if we solve this well?",
  () =>
...
export function buildProblem(project: Project): ProblemBrief {
  ...
  return {
    ...
    desiredOutcome: a[3] || "Desired outcome to be confirmed",
    businessImpact: a[3] || "Business impact to be quantified",
    evidence: [a[4], project.evidenceNotes].filter(Boolean),
    assumptions: a[4] ? [] : ["The request is currently assumption-led"],
    constraints: a[5] ? [a[5]] : [],
    successCriteria: [
